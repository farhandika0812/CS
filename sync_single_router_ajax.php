...
foreach ($users as $user) {
    try {
        ...
        // Ambil Last Logged Out dari PPP Secret
        $last_logged_out = $user['last-logged-out'] ?? '-';
        
        // Cek online status di active session
        $is_active_online = isset($active_users[$id_pelanggan]);
        $status_ping = $is_active_online ? 'ONLINE' : 'OFFLINE';
        
        $ip_modem = '-';
        $caller_id = '-';
        $uptime = '-'; // Default uptime
        
        if ($is_active_online) {
            if (isset($active_users[$id_pelanggan]['address'])) {
            ...
            // Ambil Uptime dari Active Session
            if (isset($active_users[$id_pelanggan]['uptime'])) {
                $uptime = $active_users[$id_pelanggan]['uptime'];
            }
        }
        ...
        if ($stmt_cek->fetch()) {
            // UPDATE (Tambahkan uptime & last_logged_out)
            ...
            $stmt_up->execute([
                ...
                $pppoe_profile, $rate_limit, $service_name, $router_id, $uptime, $last_logged_out, $id_pelanggan
            ]);
            $updated++;
        } else {
            // INSERT (Tambahkan uptime & last_logged_out)
            $sql = "INSERT INTO pelanggan (
                id_pelanggan, nama, alamat, ip_modem, caller_id, status_ping,
                status_pembayaran, tagihan, no_telp, status_berlangganan,
                pppoe_profile, rate_limit, service_name, router_id,
