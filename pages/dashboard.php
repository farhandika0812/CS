...
/* PPPoE column - ensure full visibility */
...
<div class="customer-table-container" style="margin-top: 25px;">
    ...
    <div style="overflow-x: auto;">
        <table class="data-table" style="width: 100%; border-collapse: collapse; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 1px 8px rgba(0,0,0,0.05);">
            <thead>
                <tr>
                    <th style="padding: 12px 12px; text-align: left; background: #f8f9fa; border-bottom: 2px solid #e9ecef; width: 20%;"><a href="#" onclick="sortTable('nama'); return false;">👤 Nama Pelanggan <?php echo $sort_column == 'nama' ? ($sort_order == 'asc' ? '↑' : '↓') : '⇅'; ?></a></th>
                    
                    <th style="padding: 12px 12px; text-align: left; background: #f8f9fa; border-bottom: 2px solid #e9ecef; width: 25%;"><a href="#" onclick="sortTable('id_pelanggan'); return false;">⚙️ PPPoE pelanggan <?php echo $sort_column == 'id_pelanggan' ? ($sort_order == 'asc' ? '↑' : '↓') : '⇅'; ?></a></th>
                    
                    <th style="padding: 12px 12px; text-align: left; background: #f8f9fa; border-bottom: 2px solid #e9ecef; width: 20%;"><a href="#" onclick="sortTable('alamat'); return false;">📍 Alamat <?php echo $sort_column == 'alamat' ? ($sort_order == 'asc' ? '↑' : '↓') : '⇅'; ?></a></th>
                    <th style="padding: 12px 12px; text-align: left; background: #f8f9fa; border-bottom: 2px solid #e9ecef; width: 15%;"><a href="#" onclick="sortTable('ip_modem'); return false;">🌐 IP Modem <?php echo $sort_column == 'ip_modem' ? ($sort_order == 'asc' ? '↑' : '↓') : '⇅'; ?></a></th>
                    <th style="padding: 12px 12px; text-align: left; background: #f8f9fa; border-bottom: 2px solid #e9ecef; width: 10%;"><a href="#" onclick="sortTable('pppoe_profile'); return false;">⚙️ Profile <?php echo $sort_column == 'pppoe_profile' ? ($sort_order == 'asc' ? '↑' : '↓') : '⇅'; ?></a></th>
                    <th style="padding: 12px 12px; text-align: center; background: #f8f9fa; border-bottom: 2px solid #e9ecef; width: 10%;"><a href="#" onclick="sortTable('status_ping'); return false;">🔗 Status <?php echo $sort_column == 'status_ping' ? ($sort_order == 'asc' ? '↑' : '↓') : '⇅'; ?></a></th>
                </tr>
            </thead>
            <tbody>
                        ...
                        <tr style="border-bottom: 1px solid #e9ecef;">
                            <td style="padding: 10px 12px; font-weight: 500; word-break: break-word;">
                                <a href="#" onclick="showDetailModal(
                                    ...
                                    '<?php echo htmlspecialchars(addslashes($pelanggan['status_pembayaran'] ?? 'belum bayar')); ?>',
                                    '<?php echo htmlspecialchars(addslashes($pelanggan['no_telp'] ?? '-')); ?>',
                            ...
                            <td style="padding: 10px 12px; font-family: monospace; word-break: break-all; white-space: normal; font-size: 12px;">
                                <?php echo htmlspecialchars($pelanggan['id_pelanggan']); ?>
                            </td>
                            ...
                            <td style="padding: 10px 12px;">
                                <?php if (!empty($pelanggan['ip_modem']) && $pelanggan['ip_modem'] != '-' && filter_var($pelanggan['ip_modem'], FILTER_VALIDATE_IP)): ?>
                                    <a href="#" onclick="showRemoteModal('<?php echo htmlspecialchars($pelanggan['ip_modem']); ?>', '<?php echo htmlspecialchars($pelanggan['nama']); ?>'); return false;" class="ip-link" style="font-size: 11px; padding: 3px 8px;">
                                        🖥️ <?php echo htmlspecialchars($pelanggan['ip_modem']); ?>
                                    </a>
                                    <div style="font-size: 10px; color: #7f8c8d; margin-top: 4px; font-family: monospace; padding-left: 5px;">
                                        🔑 <?php echo htmlspecialchars($pelanggan['caller_id'] ?? '-'); ?>
                                    </div>
                                <?php else: ?>
                                    <span style="color: #999; font-size: 11px;">❓ <?php echo htmlspecialchars($pelanggan['ip_modem']); ?></span>
                                <?php endif; ?>
                            </td>

                            <td style="padding: 10px 12px; font-family: monospace; word-break: break-word; font-size: 11px;">
                                <?php 
                            ...
                            <td style="padding: 10px 12px; text-align: center;">
                                <div onclick="showPingModal('<?php echo htmlspecialchars($pelanggan['ip_modem']); ?>', '<?php echo htmlspecialchars($pelanggan['nama']); ?>', '<?php echo htmlspecialchars($pelanggan['router_id'] ?? 0); ?>')" 
                                    class="<?php echo $pelanggan['status_ping'] == 'ONLINE' ? 'status-online' : 'status-offline'; ?>" 
                                    title="Klik untuk melihat grafik ping" style="width: 28px; height: 28px; font-size: 13px; margin: 0 auto;">
                                    <i class="fas fa-sitemap" style="color: white; font-size: 13px;"></i>
                                </div>
                                
                                <?php if ($pelanggan['status_ping'] == 'ONLINE'): ?>
                                    <?php if (!empty($pelanggan['uptime']) && $pelanggan['uptime'] != '-'): ?>
                                        <div style="font-size: 9px; color: #27ae60; margin-top: 4px; font-weight: bold; white-space: nowrap;">
                                            ⏱️ <?php echo htmlspecialchars($pelanggan['uptime']); ?>
                                        </div>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <?php if (!empty($pelanggan['last_logged_out']) && $pelanggan['last_logged_out'] != '-'): ?>
                                        <div style="font-size: 9px; color: #e74c3c; margin-top: 4px; white-space: nowrap;">
                                            🕒 <?php echo htmlspecialchars($pelanggan['last_logged_out']); ?>
                                        </div>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </td>
...
<div id="detailModal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="info-row"><div class="info-label">⏱️ Uptime</div><div class="info-value" id="detailUptime">-</div></div>
            <div class="info-row"><div class="info-label">🕒 Last Logout</div><div class="info-value" id="detailLastLogout">-</div></div>
        </div>
        <div class="modal-footer">
...
let pingData = { ip: '', nama: '', router_id: '', history: [] };
...
function addSyncLog(message, type = 'info') {
    ...
    logEntry.innerHTML = `<span class="sync-log-icon">${icon}</span><span class="sync-log-message">${message}</span><span class="sync-log-time">${time}</span>`;
...
function updateSyncProgress(current, total, message) {
    ...
    document.getElementById('syncProgressText').innerHTML = `${message} (${current}/${total})`;
}
...
function closeSyncModal() {
    if (!syncInProgress) document.getElementById('syncModal').style.display = 'none';
    else showToast('Sinkronisasi sedang berjalan, harap tunggu!', 'warning');
}
...
async function syncAllRoutersWithModal() {
    if (syncInProgress) { showToast('Sinkronisasi sedang berjalan!', 'warning'); return; }
    
    const syncBtn = document.getElementById('syncRouterBtn');
    const originalText = syncBtn.innerHTML;
    syncBtn.disabled = true; syncBtn.innerHTML = '⏳ Menyinkronkan...'; syncBtn.classList.add('disabled');
    showSyncModal(); syncInProgress = true;
    
    try {
        addSyncLog('Mengambil daftar router aktif...', 'info');
        const response = await fetch('get_active_routers.php', {
            method: 'GET', headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }, cache: 'no-store'
        });
        const routers = await response.json();
        
        if (!routers || routers.length === 0) {
            addSyncLog('Tidak ada router aktif!', 'error'); updateSyncProgress(0, 1, 'Gagal!');
            document.getElementById('syncCloseBtn').disabled = false; syncInProgress = false;
            showToast('Tidak ada router aktif', 'error');
            syncBtn.disabled = false; syncBtn.innerHTML = originalText; syncBtn.classList.remove('disabled'); return;
        }
        
        const total = routers.length; let completed = 0, successCount = 0, failCount = 0, totalUpdated = 0, totalInserted = 0;
        addSyncLog(`Ditemukan ${total} router aktif, memulai sinkronisasi...`, 'info');
        
        for (let i = 0; i < routers.length; i++) {
            const router = routers[i]; const current = i + 1;
            ...
            try {
                const syncResponse = await fetch(`sync_single_router_ajax.php?router_id=${router.id}`, { method: 'GET', headers: { 'Content-Type': 'application/json' }, cache: 'no-store' });
                const result = await syncResponse.json();
                
                if (result.status === 'SUCCESS') {
                    successCount++; totalUpdated += result.updated || 0; totalInserted += result.inserted || 0;
                    ...
                    failCount++; addSyncLog(`❌ ${router.nama} - Gagal: ${result.message || 'Unknown error'}`, 'error');
                }
            } catch (error) {
                failCount++; addSyncLog(`❌ ${router.nama} - Error: ${error.message}`, 'error');
            }
            completed++; updateSyncProgress(completed, total, `Selesai ${completed}/${total}`);
            await new Promise(resolve => setTimeout(resolve, 500));
        }
        
        updateSyncProgress(total, total, 'Selesai!'); updateSyncSummary(successCount, failCount, total, totalUpdated, totalInserted);
        ...
        document.getElementById('syncCloseBtn').disabled = false; syncInProgress = false;
        setTimeout(() => { closeSyncModal(); window.location.reload(); }, 2000);
        
    } catch (error) {
        console.error('Error:', error); addSyncLog(`❌ Error: ${error.message}`, 'error'); updateSyncProgress(0, 1, 'Error!');
        document.getElementById('syncCloseBtn').disabled = false; syncInProgress = false;
        showToast('❌ Gagal terhubung ke server: ' + error.message, 'error');
    } finally {
        syncBtn.disabled = false; syncBtn.innerHTML = originalText; syncBtn.classList.remove('disabled');
    }
}
...
function sortTable(column) {
    ...
    document.getElementById('sort_input').value = column; document.getElementById('order_input').value = newOrder;
    document.getElementById('page_input').value = 1; document.getElementById('sortForm').submit();
}
...
function goToPage(page) {
    ...
    document.getElementById('page_input').value = page; document.getElementById('sortForm').submit();
}

function resetSearch() {
    document.getElementById('sort_input').value = '<?php echo $sort_column ?? ""; ?>';
    document.getElementById('order_input').value = '<?php echo $sort_order ?? ""; ?>';
    document.getElementById('page_input').value = 1; document.getElementById('search_input').value = '';
...
function showDetailModal(nama, alamat, pembayaran, telp, service, statusLangganan, routerId, uptime, lastLogout) {
    ...
    if (telp && telp !== '-' && telp.trim() !== '') {
        ...
        if (cleanTelp.startsWith('0')) cleanTelp = '62' + cleanTelp.substring(1); else if (!cleanTelp.startsWith('62')) cleanTelp = '62' + cleanTelp;
        currentWhatsAppNumber = cleanTelp; document.getElementById('detailTelp').innerHTML = telp + ' <span style="font-size:10px;color:#25D366;">(WA)</span>';
    } else {
        currentWhatsAppNumber = ''; document.getElementById('detailTelp').innerHTML = '<span style="color:#e74c3c;">❌ Tidak tersedia</span>';
    }
    document.getElementById('detailService').innerText = service;
    
    let statusHtml = statusLangganan.toLowerCase() == 'aktif' ? '<span class="status-badge aktif">🟢 Aktif</span>' : (statusLangganan.toLowerCase() == 'nonaktif' ? '<span class="status-badge nonaktif">🔴 Nonaktif</span>' : '<span class="status-badge suspend">🟡 Suspend</span>');
    document.getElementById('detailStatusLangganan').innerHTML = statusHtml; document.getElementById('detailRouterId').innerText = routerId;
    
    document.getElementById('detailUptime').innerHTML = uptime && uptime !== '-' ? `<span style="color:#27ae60; font-weight:bold; font-family:monospace;">${uptime}</span>` : '-';
    document.getElementById('detailLastLogout').innerHTML = lastLogout && lastLogout !== '-' ? `<span style="color:#e74c3c; font-family:monospace;">${lastLogout}</span>` : '-';
...
function sendWhatsApp() {
    ...
    } else { showToast('Nomor telepon tidak valid', 'warning'); }
}

function showRemoteModal(ip, nama) {
    remoteData = { ip, nama }; document.getElementById('modalNama').innerText = nama; document.getElementById('modalIp').innerText = ip;
    document.getElementById('remoteModal').style.display = 'block';
}

function remoteModem() { if (remoteData.ip) { window.open('http://' + remoteData.ip, '_blank'); closeModal('remoteModal'); } }
function closeModal(modalId) { document.getElementById(modalId).style.display = 'none'; }
function closePingModal() { if (pingInterval) clearInterval(pingInterval); document.getElementById('pingModal').style.display = 'none'; }

function showPingModal(ip, nama, routerId) {
    if (!ip || ip === '-') { showToast('IP Modem tidak valid', 'warning'); return; }
    pingData = { ip: ip, nama: nama, router_id: routerId, history: [] };
    
    document.getElementById('pingModalNama').innerText = nama; document.getElementById('pingModalIp').innerText = ip;
    document.getElementById('pingModal').style.display = 'block';
    
    document.getElementById('pingMin').innerText = '-'; document.getElementById('pingMax').innerText = '-';
    document.getElementById('pingAvg').innerText = '-'; document.getElementById('pingCurrent').innerText = '-';
    document.getElementById('pingStatus').className = 'ping-status online'; document.getElementById('pingStatus').innerHTML = '🌐 Menghubungkan...';
    
    if (pingInterval) clearInterval(pingInterval);
    fetchPingData(); pingInterval = setInterval(fetchPingData, 3000);
}

async function fetchPingData() {
    if (!pingData.ip || !pingData.router_id) return;
    try {
        const response = await fetch(`ping_modem_ajax.php?ip=${pingData.ip}&router_id=${pingData.router_id}`);
        const data = await response.json();
        
        let isOnline = (data.status === 'ONLINE'); 
        let numericLatency = null;
        
        if (isOnline) {
            let timeStr = data.time;
            
            if (timeStr.includes('ms')) {
                numericLatency = parseInt(timeStr.split('ms')[0]); 
            } else if (timeStr.includes('us')) {
                numericLatency = 1; 
            } else if (timeStr.includes('s')) {
                numericLatency = parseInt(timeStr.split('s')[0]) * 1000;
            } else {
                numericLatency = parseInt(timeStr.replace(/[^0-9]/g, '')) || 1;
            }
        }
        ...
        console.error("Gagal melakukan live ping:", error); 
    }
}

function updatePingChartUI() {
    let history = pingData.history; if (history.length === 0) return;
    let valid = history.filter(p => p.latency !== null).map(p => p.latency); let currNode = history[history.length-1];

    if (valid.length === 0) {
        document.getElementById('pingMin').innerHTML = '-'; document.getElementById('pingMax').innerHTML = '-';
        document.getElementById('pingAvg').innerHTML = '-'; document.getElementById('pingCurrent').innerHTML = 'RTO';
        document.getElementById('pingStatus').className = 'ping-status offline'; document.getElementById('pingStatus').innerHTML = '🔌 Offline';
    } else {
        let minLat = Math.min(...valid), maxLat = Math.max(...valid), avgLat = Math.round(valid.reduce((a,b)=>a+b,0)/valid.length);
        document.getElementById('pingMin').innerHTML = minLat + ' ms'; document.getElementById('pingMax').innerHTML = maxLat + ' ms';
        document.getElementById('pingAvg').innerHTML = avgLat + ' ms'; document.getElementById('pingCurrent').innerHTML = (currNode.status === 'online' ? currNode.raw_time : 'RTO');
        document.getElementById('pingStatus').className = 'ping-status online'; document.getElementById('pingStatus').innerHTML = '🌐 Online';
    }
...
function renderLineChart(history) {
    let canvas = document.getElementById('pingLineChart'); if (!canvas) return;
    let ctx = canvas.getContext('2d'); let container = canvas.parentElement;
    let width = container.clientWidth - 20; let height = 140;
    canvas.width = width; canvas.height = height; ctx.clearRect(0, 0, width, height);
    if (history.length === 0) {
        ctx.fillStyle = '#999'; ctx.font = '12px sans-serif'; ctx.textAlign = 'center'; ctx.fillText('Tidak ada data', width/2, height/2); return;
    }
    let padding = { left: 35, right: 10, top: 12, bottom: 20 };
    let chartWidth = width - padding.left - padding.right; let chartHeight = height - padding.top - padding.bottom;
    let stepX = chartWidth / (history.length - 1); let maxLat = Math.max(...history.filter(p => p.latency).map(p => p.latency), 80);
    ...
    let first = true, points = [];
    for (let i = 0; i < history.length; i++) {
        let x = padding.left + stepX * i; let y = padding.top + chartHeight;
        if (history[i].latency !== null && history[i].status === 'online') { y = padding.top + chartHeight - (history[i].latency / maxLat) * chartHeight; }
    ...
    for (let i = 0; i < points.length; i++) {
        let p = points[i]; ctx.beginPath();
        if (p.latency !== null && p.status === 'online') {
            ctx.fillStyle = '#3498db'; ctx.arc(p.x, p.y, 4, 0, 2 * Math.PI); ctx.fill();
            ctx.fillStyle = 'white'; ctx.arc(p.x, p.y, 2, 0, 2 * Math.PI); ctx.fill();
            ctx.fillStyle = '#2c3e50'; ctx.font = '9px monospace'; ctx.textAlign = 'center'; ctx.fillText(p.latency + 'ms', p.x, p.y - 7);
        } else {
            ctx.fillStyle = '#e74c3c'; ctx.arc(p.x, p.y, 4, 0, 2 * Math.PI); ctx.fill();
            ctx.fillStyle = '#e74c3c'; ctx.font = '8px monospace'; ctx.textAlign = 'center'; ctx.fillText('offline', p.x, p.y - 7);
        }
        if (i % 3 === 0 || i === history.length - 1) {
            ctx.fillStyle = '#aaa'; ctx.font = '8px monospace'; ctx.textAlign = 'center'; ctx.fillText(p.time || (i+1+''), p.x, height - 5);
        }
    }
}
...
window.onclick = function(event) {
    ...
    if (event.target == document.getElementById('syncModal')) { if (!syncInProgress) closeSyncModal(); }
}

document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.querySelector('input[name="search"]'); const searchForm = document.getElementById('searchForm'); let debounceTimer;
    if (searchInput && searchForm) {
        searchInput.addEventListener('input', function(e) {
            clearTimeout(debounceTimer); 
            const sortSearchInput = document.getElementById('search_input'); if (sortSearchInput) sortSearchInput.value = e.target.value;
            debounceTimer = setTimeout(function() {
                const tbody = document.querySelector('.data-table tbody'); if (tbody) tbody.style.opacity = '0.3'; 
                const formData = new FormData(searchForm);
                fetch(window.location.href, { method: 'POST', body: formData }).then(response => response.text()).then(html => {
                    const parser = new DOMParser(); const doc = parser.parseFromString(html, 'text/html');
                    const currentTableWrapper = document.querySelector('.data-table').parentElement; const newTableWrapper = doc.querySelector('.data-table').parentElement;
                    if (currentTableWrapper && newTableWrapper) {
                        currentTableWrapper.innerHTML = newTableWrapper.innerHTML; currentTableWrapper.querySelector('.data-table tbody').style.opacity = '1';
                    }
                    let sibling = currentTableWrapper.nextElementSibling; while (sibling) { let next = sibling.nextElementSibling; sibling.remove(); sibling = next; }
                    let newSibling = newTableWrapper.nextElementSibling; while (newSibling) { currentTableWrapper.parentElement.appendChild(newSibling.cloneNode(true)); newSibling = newSibling.nextElementSibling; }
                    const currentResetBtn = searchForm.querySelector('.btn-warning'); const newResetBtn = doc.getElementById('searchForm').querySelector('.btn-warning');
                    if (newResetBtn && !currentResetBtn) {
                        const searchBtn = searchForm.querySelector('.btn-primary'); searchBtn.insertAdjacentElement('afterend', newResetBtn.cloneNode(true));
                    } else if (!newResetBtn && currentResetBtn) { currentResetBtn.remove(); }
                }).catch(error => { console.error('Error saat Live Search:', error); if (tbody) tbody.style.opacity = '1'; });
            }, 400); 
        });
